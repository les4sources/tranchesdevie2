class Admin::BakeDaysController < Admin::BaseController
  ...
  def index
    ...
    @future_bake_days = BakeDay.future.includes(:baking_artisans, :pickup_locations, orders: { order_items: { product_variant: [ :mold_type, { product: { product_flours: :flour } } ] } }).order(:baked_on)
    ...
    @past_bake_days = past_bake_days.includes(:baking_artisans, :pickup_locations, orders: { order_items: { product_variant: [ :mold_type, { product: { product_flours: :flour } } ] } }).order(:baked_on)
