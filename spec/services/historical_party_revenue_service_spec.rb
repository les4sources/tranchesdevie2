...
RSpec.describe HistoricalPartyRevenueService do
  ...
  # Juillet réel : 35 adultes, 16 enfants, 13 boules sourciers, frais 27,13 €. (Garnitures hors app.)
  let(:event) do
    create(:party_event, :public_party,
           title: "Pizza Party de juillet", held_on: Date.new(2026, 7, 18),
           historical_source: "billetweb", historical_adults: 35, historical_children: 16,
  ...
  context "sans coûtant configuré (coûtant = 0)" do
    ...
    it "compte les participants et les sourciers" do
      expect(result.persons).to eq(51)
      expect(result.adults).to eq(35)
      expect(result.children).to eq(16)
      expect(result.sourciers).to eq(13)
    end

    it "calcule le CA des places (446 €) — sourciers non payants exclus" do
      expect(result.sale_cents).to eq(44_600) # 35×1000 + 16×600
    end

    it "applique le barème public sur les places : boulangers 216,30 € / 4S 229,70 €" do
      expect(result.bakers_places_cents).to eq(21_630) # 35×490 + 16×280
      expect(result.four_sources_cents).to eq(22_970)  # 35×510 + 16×320
    end

    it "crédite les sourciers aux boulangers à 2 €/boule" do
      expect(result.bakers_sourciers_cents).to eq(2_600)          # 13 × 200
      expect(result.bakers_cents).to eq(21_630 + 2_600)           # places + sourciers = 242,30 €
    end

    it "réconcilie les PLACES : boulangers_places + 4S = CA − coûtant" do
      expect(result.bakers_places_cents + result.four_sources_cents).to eq(result.sale_cents - result.dough_cost_cents)
    end
    ...
    it "expose les frais et le net réellement encaissé par 4S" do
      ...
      expect(result.four_sources_effective_cents).to eq(17_657)     # 41887 − 24230 (dû aux boulangers)
    end
  end
  ...
  context "avec un coûtant configuré sur le pâton adulte" do
    ...
    it "déduit le coûtant de la marge boulangers (places)" do
      # adulte: marge 674 → boulangers 472 ; enfant inchangé 280
      expect(result.bakers_places_cents).to eq(35 * 472 + 16 * 280) # 16 520 + 4 480 = 21 000
